const {
  ...
  getPgDarkPoolTheDeepNoteDepositInputError,
...
async function pgDarkPoolTorosYieldDeposit(req, res) {
  const inputError = getPgDarkPoolTorosYieldDepositInputError(req.body)
  if (inputError) {
    console.log('Invalid input:', inputError)
    return res.status(400).json({ error: inputError })
  }
  ...
  const id = await postJob({
    type: jobType.PG_DARKPOOL_TOROS_YIELD_DEPOSIT,
    request: req.body,
  })
  return res.json({ id })
}

async function pgDarkPoolTorosYieldWithdraw(req, res) {
  const inputError = getPgDarkPoolTorosYieldWithdrawInputError(req.body)
  if (inputError) {
    console.log('Invalid input:', inputError)
    return res.status(400).json({ error: inputError })
  }
  ...
  const id = await postJob({
    type: jobType.PG_DARKPOOL_TOROS_YIELD_WITHDRAW,
    request: req.body,
  })
  return res.json({ id })
}

async function pgDarkPoolTheDeepNoteDeposit(req, res) {
  const inputError = getPgDarkPoolTheDeepNoteDepositInputError(req.body)
  if (inputError) {
    console.log('Invalid input:', inputError)
    return res.status(400).json({ error: inputError })
  }
  ...
  const id = await postJob({
    type: jobType.PG_DARKPOOL_THE_DEEP_NOTE_DEPOSIT,
    request: req.body,
  })
  return res.json({ id })
}
